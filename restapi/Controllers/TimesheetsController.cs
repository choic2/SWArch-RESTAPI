...
namespace restapi.Controllers
{
    ...
    public class TimesheetsController : Controller
    {
        ...
        [ProducesResponseType(typeof(UnauthorizedActionError), 403)]
        [ProducesResponseType(404)]
        [ProducesResponseType(typeof(InvalidStateError), 409)]
        [ProducesResponseType(typeof(EmptyTimecardError), 409)]
        public IActionResult Submit(string ownid, string id, [FromBody] Submittal submittal)
        {
            Timecard timecard = Database.Find(id);
            Timecard SubmittersTimecard = Database.Find(ownid);

            if (SubmittersTimecard.Resource != timecard.Resource)
        ...
        [ProducesResponseType(typeof(InvalidStateError), 409)]
        [ProducesResponseType(typeof(EmptyTimecardError), 409)]
        public IActionResult Cancel(string ownid, string id, [FromBody] Cancellation cancellation)
        {
            Timecard timecard = Database.Find(id);
            Timecard CancellersTimecard = Database.Find(ownid);

            if (timecard.Status != TimecardStatus.Draft &&
                CancellersTimecard.Resource == timecard.Resource)
                return StatusCode(403, new UnauthorizedActionError() { });
            else if (CancellersTimecard.Resource == timecard.Resource)
                return StatusCode(403, new UnauthorizedActionError() { });
            else
            {
                if (timecard != null)
        ...
        [ProducesResponseType(typeof(EmptyTimecardError), 409)]
        [ProducesResponseType(typeof(UnauthorizedActionError), 403)]
        public IActionResult Approve(string ownid, string id, [FromBody] Approval approval)
        {
            Timecard timecard = Database.Find(id);
            Timecard approversTimecard = Database.Find(ownid);
